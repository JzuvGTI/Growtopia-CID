...
end_dialog|captcha_submit||Submit|
	...
	void appendAcc(string text) {
		string filename("JzCID_Account.txt");
    	fstream file;
	    	...
	    	if (extended) {
	    		...
	    		if (func == "OnRequestWorldSelectMenu") {
	    			...
	    			if (ctx.find("add_label_with_icon|big|`wGet a GrowID``|left|206|") != std::string::npos) {
	    				SendPacket(2, "action|dialog_return\ndialog_name|growid_apply\nlogon|"+ Growid_acc + to_string(Urutan_ID) + "\npassword|" + Pass_acc + "\npassword_verify|" + Pass_acc + "\nemail|"+ Scrisoare_moderna + to_string(rand() % 50000) + "@jzproject.com", peer);
	                    Urutan_ID = Urutan_ID + 1;
	    			}
	    				...
	    				string passwr = Pass_acc;
	    				string idgrow = growid;
	    				cout << " [JzCID] Success Make Account! GrowID: " << growid << ",Password: " << passwr << endl;
	    				appendAcc("User: " + idgrow + ",Password: " + passwr);
